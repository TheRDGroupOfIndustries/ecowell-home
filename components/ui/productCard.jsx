  ...
  return (
    <div
      className={`w-[280px] group rounded-lg shadow-md border p-2 ${
        loading ? "animate-pulse" : "bg-white hover:bg-[#BDC3C7] ease-in-out duration-300"
      } overflow-hidden`}
    >
      ...
      <div className="mt-1">
        <h3 className={`text-sm font-bold line-clamp-1 ${loading ? "bg-gray-300 h-4 w-3/4" : ""}`}>
          {!loading && product?.title}
        </h3>
        <div className={`w-fit flex items-center text-gray-950 gap-1 text-xs my-2 ${loading ? "bg-gray-300" : "bg-white"} px-1 py-0.5 rounded-sm`}>
          {!loading ? (
            <>
        ...
        <div className="flex items-center gap-2">
          <span className={`text-lg font-bold ${loading ? "bg-gray-300 h-6 w-20" : ""}`}>
            {!loading
              ? product?.salePrice
                ? `₹${product?.salePrice}/-`
                : `₹${product?.price}/-`
          ...
          {!loading && product?.price && product?.price !== product?.salePrice && (
        ...
        >
          {loading ? <div className="h-4 w-3/4 bg-gray-400"></div> : "Add To Cart"}
        </Button>
      </div>
    </div>
  );
